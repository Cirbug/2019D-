...
void AD9954_Init(void)
{
	...
	// 设置OSK引脚为高电平，使Manual OSK模式下的幅度输出生效
	// 如果OSK引脚为低，输出会被静音（幅度为0）
	AD9954_OSK_HIGH();
...
** 出口参数 ：无
** 隐含控制   			AD9954_PS0_Pin: 0   1	0	 1
** 管脚参数： 			AD9954_PS1_Pin: 0   0	1	 1
** 对应控制 RAM段：		 0   1	2	 3
** 函数说明 ：在四个RAM区各设置了一个频率值，通过改变AD9954_PS0_Pin和AD9954_PS1_Pin的电平选择对应的RAM端输出相应的频率值来实现FSK，也可以实现二项的FSK；
**            通过设置控制AD9954_PS0_Pin,AD9954_PS1_Pin管脚的电平就可以将二进制的编码转化为FSK信号输出
*********************************************************************************************************/
void AD9954_SetFSK(double f1,double f2,double f3,double f4,uint16_t Ampli)   
{
  ...
  fdata[3]=FTW_Vau;
	...
	AD9954_PS1_LOW();AD9954_PS0_HIGH();
	FTW_Vau=Get_FTW(f2); 
  fdata[0]=FTW_Vau>>24;
  fdata[1]=FTW_Vau>>16;
  fdata[2]=FTW_Vau>>8;
  fdata[3]=FTW_Vau;
	...
	AD9954_PS1_HIGH();AD9954_PS0_LOW();
	FTW_Vau=Get_FTW(f3); 
  fdata[0]=FTW_Vau>>24;
  fdata[1]=FTW_Vau>>16;
  fdata[2]=FTW_Vau>>8;
  fdata[3]=FTW_Vau;
	...
	AD9954_PS1_HIGH();AD9954_PS0_HIGH();
	FTW_Vau=Get_FTW(f4); 
  fdata[0]=FTW_Vau>>24;
  fdata[1]=FTW_Vau>>16;
  fdata[2]=FTW_Vau>>8;
...
** 管脚参数： 			AD9954_PS1_Pin: 0   0	1	 1
** 对应控制 RAM段：		 0   1	2	 3
** 函数说明 ：在四个RAM区各设置了一个频率值，通过改变AD9954_PS0_Pin和AD9954_PS1_Pin的电平选择对应的RAM端输出相应的频率值来实现PSK，也可以实现二项的PSK；
**            通过设置控制AD9954_PS0_Pin,AD9954_PS1_Pin管脚的电平就可以将二进制的编码转化为PSK信号输出
*********************************************************************************************************/
...
**						扫频总时间=总扫描频点数*T
**						AD9954_PS0_Pin脚控制扫频方向，AD9954_PS0_Pin=1向上扫频，AD9954_PS0_Pin=0向下扫频
*********************************************************************************************************/
