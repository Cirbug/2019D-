...
/* 显示字符/字符串/数字 */
void lcd_show_char(uint16_t x, uint16_t y, char ch, uint8_t size, uint16_t color, uint16_t bg_color);
void lcd_show_string(uint16_t x, uint16_t y, uint8_t size, char *str, uint16_t color, uint16_t bg_color);
void lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color, uint16_t bg_color);
void lcd_show_float(uint16_t x, uint16_t y, double num, uint8_t int_len, uint8_t dec_len, uint8_t size, uint16_t color, uint16_t bg_color);
...
void lcd_show_char(uint16_t x, uint16_t y, char ch, uint8_t size, uint16_t color, uint16_t bg_color)
{
    uint8_t i, j;
    uint16_t temp;
    const unsigned char *chbase = NULL;
    uint8_t chwidth, chbytes_per_col;
    
    if (ch < ' ' || ch > '~') ch = ' ';
    ch -= ' ';
    
    switch (size) {
        case 12: chbase = asc2_1206[ch]; chwidth = 6;  chbytes_per_col = 2; break;
        case 16: chbase = asc2_1608[ch]; chwidth = 8;  chbytes_per_col = 2; break;
        case 24: chbase = asc2_2412[ch]; chwidth = 12; chbytes_per_col = 3; break;
        case 32: chbase = asc2_3216[ch]; chwidth = 16; chbytes_per_col = 4; break;
        default: return;
    }
    
    for (i = 0; i < chwidth; i++) {
        for (j = 0; j < chbytes_per_col; j++) {
            temp = chbase[i * chbytes_per_col + j];
            for (uint8_t k = 0; k < 8; k++) {
                if (temp & (0x80 >> k)) {
                    lcd_draw_point(x + i, y + j * 8 + k, color);
                } else {
                    lcd_draw_point(x + i, y + j * 8 + k, bg_color);
                }
            }
        }
    }
...
void lcd_show_string(uint16_t x, uint16_t y, uint8_t size, char *str, uint16_t color, uint16_t bg_color)
{
    uint8_t chwidth;
    switch (size) {
        case 12: chwidth = 6;  break;
        case 16: chwidth = 8;  break;
        case 24: chwidth = 12; break;
        case 32: chwidth = 16; break;
        default: return;
    }
    
    while (*str != '\0') {
        lcd_show_char(x, y, *str, size, color, bg_color);
        x += chwidth;
        str++;
    }
}
...
void lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color, uint16_t bg_color)
{
    ...
    lcd_show_string(x, y, size, buf, color, bg_color);
}
...
void lcd_show_float(uint16_t x, uint16_t y, double num, uint8_t int_len, uint8_t dec_len, uint8_t size, uint16_t color, uint16_t bg_color)
{
    ...
    temp = num - (double)int_part;
    if (temp < 0) temp = -temp;
    dec_part = (uint32_t)(temp * pow10(dec_len));
    
    lcd_show_num(x, y, int_part, int_len, size, color, bg_color);
    ...
    lcd_show_char(x + int_len * chwidth, y, '.', size, color, bg_color);
    lcd_show_num(x + (int_len + 1) * chwidth, y, dec_part, dec_len, size, color, bg_color);
}
